...
static inline void VNCRuntimeGotFrameBufferUpdate(rfbClient* cl, int x, int y, int w, int h, SDL_Texture* sdlTexture, SDL_Renderer* sdlRenderer, SDL_Window* sdlWindow) {
    ...
    if (vncClient && cl->appData.useRemoteCursor) {
        ...
        // 简单的边缘检测：只有在冷却时间结束时才触发
        BOOL canTriggerEdgeFollow = (g_edgeFollowCooldown == 0);
        ...
        if (scaledWidth > renderWidth) {
            ...
            if (canTriggerEdgeFollow && screenMouseX < CURSOR_EDGE_THRESHOLD && currentOffsetX < 0) {
            ...
            else if (canTriggerEdgeFollow && screenMouseX > (renderWidth - CURSOR_EDGE_THRESHOLD) && 
        ...
        if (scaledHeight > renderHeight) {
            ...
            if (canTriggerEdgeFollow && screenMouseY < CURSOR_EDGE_THRESHOLD && currentOffsetY < 0) {
            ...
            else if (canTriggerEdgeFollow && screenMouseY > (renderHeight - CURSOR_EDGE_THRESHOLD) && 
